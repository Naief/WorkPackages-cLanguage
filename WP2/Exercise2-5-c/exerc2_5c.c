...
void create_random(int *tab){
    ...
    for (int i=0; i<MAX; i++) { // Creates a random number for each entry in the table
...
void count_frequency(int *tab, int *freq){
    ...
    for(int i = 0; i<MAXNUMBER+1; i++){ // Initializes the frequency array. Prevents errors.
        freq[i] = 0;
    }

    for(int i = 0; i<MAX; i++){ // This is the loop which actually counts the numbers.
        freq[tab[i]]++;
    }
}

void draw_histogram(int *freq){ // The part that prints Xes for each occurance of a number
    //int count = 0;// Testing purposes
    for(int i = 0; i < MAXNUMBER+1; i++) {
        if(freq[i] >0 ) { // If the frequencecy is higher than zero then the number and its frequency is printed.
            printf("\n%i", i);
            for (int x = 0; x < freq[i]; x++) {// Prints a X for each occurance of the number
                //count++; // Testing purposes
                printf("x");
            }
        }
    }
    //printf("\ncount: %d Xes", count);// Testing purposes
}